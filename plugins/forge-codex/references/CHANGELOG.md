...
claims" and "Run `Codex --version`" for a Codex list.
...
as unreachable there as `~/.codex/forge/rules/requirements/tables.md` was, and the blocked-run rule
...
about the same document. That is the exact failure v4.7.0 named and repaired for the Codex side.
...
Both version stamps now read 4.7.4. `test-forge-parity.ps1` passes: 113 shared skills, 114 Codex
...
them, and `setup.sh` copies them to `~/.codex/forge/skills` and `~/.codex/forge/commands`. Forge's `install.sh`
...
- **`setup.sh` skips the bundle where Forge is installed.** A symlink at `~/.codex/forge/skills` or
  `~/.codex/forge/commands` is the signal that Forge owns those names, and the bundle is redundant there
