...
> **Company-aware:** When `active_company` is set in `~/.codex/forge/preferences.md` (configured by `$company-add`), `registry.md` updates are written to `~/.codex/forge/companies/[active_company]/registry.md` and idea/project files are resolved from the company directory.
...
2. **Infer entity type** from Forge ID prefix:
   - `IDEA-` → idea, file at `~/.codex/forge/ideas/active/[slug]$idea.md`
   - `PROJ-` → project, file at `~/.codex/forge/knowledge/projects/[slug]/Wiki/overview.md`
3. **Look up file path** from `~/.codex/forge/registry.md` using the Forge ID
...
7. **Update `~/.codex/forge/registry.md`** — write Jira ID to the `External ID` column for the Forge ID row
...
In `~/.codex/forge/registry.md`, the Idea Registry and Project Registry tables gain an `External ID` column:

```markdown
| IDEA-003 | My Idea | Active | PROJ-123 (Jira ticket) | ~/.codex/forge/ideas/active/my-idea/ |
| PROJ-001 | My Project | Active | EPIC-456 (Jira epic) | ~/.codex/forge/knowledge/projects/my-project/ |
```
