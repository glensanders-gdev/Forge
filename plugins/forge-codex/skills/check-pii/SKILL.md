...
Read `~/.codex/forge/companies/[active_company]/config.md` (if set) for `ai_data_restrictions`. If set, surface at the top of the report:
...
**Custom categories:** Read `~/.codex/forge/knowledge/company/pii-categories.md` if it exists. Merge with defaults.

### 3. Load System Context

Read relevant `~/.codex/forge/knowledge/systems/*/overview.md` and `known-issues.md` — note any system-level encryption, access control, or known PII handling gaps. This informs the handling assessment for Necessary PII findings.
...
- **No report found** → warn: "No PII report found. Run `check-pii` before approving."
...
Create `~/.codex/forge/knowledge/company/pii-categories.md` to extend the default categories:
