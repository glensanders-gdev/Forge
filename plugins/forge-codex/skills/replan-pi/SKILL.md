...
- User explicitly runs `replan-pi`
- `$create-project` prompts a PI replan after a new project is added
- `$replan-sprint` surfaces PI-level impact
...
Read `~/.codex/forge/priorities.md` — current feature priority ranking.
...
1. Update `~/.codex/forge/pi/[current-pi]/plan.md` in place:
   - Add injected project/feature to target release
   - Move any displaced features to next release
   - Update release capacity notes
2. Update `~/.codex/forge/sprints/calendar.md` if sprint assignments change
...
On second `CONFIRM`:
- Update `~/.codex/forge/pi/[current-pi]/stakeholder.md` in place
