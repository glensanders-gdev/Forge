---
name: "company-git"
description: "Connect the local company knowledge directory to a company-approved GitHub remote. One-time setup — initialises a git repo in ~/.codex/forge/companies/[name]/, adds the remote, and either pushes initial stubs (empty remote) or pulls existing team content (populated remote). Use when user runs $company-git [remote-url] after $company-add."
metadata:
  category: company
...
Connect the company knowledge directory at `~/.codex/forge/companies/[active_company]/` to a
...
Check `~/.codex/forge/companies/[active_company]/` exists. If not:

```
❌ Company directory not found at ~/.codex/forge/companies/[active_company]/.
...
Check if `~/.codex/forge/companies/[active_company]/.git` exists. If it does:
...
```bash
git -C ~/.codex/forge/companies/[active_company] init
git -C ~/.codex/forge/companies/[active_company] remote add origin [url]
```

If already a git repo (confirmed overwrite above):
```bash
git -C ~/.codex/forge/companies/[active_company] remote set-url origin [url]
```
...
Write `~/.codex/forge/companies/[active_company]/.gitignore`:
...
git -C ~/.codex/forge/companies/[active_company] commit -m "chore: initial company knowledge scaffold"
git -C ~/.codex/forge/companies/[active_company] branch -M main
git -C ~/.codex/forge/companies/[active_company] push -u origin main
```
...
```bash
git -C ~/.codex/forge/companies/[active_company] fetch origin
git -C ~/.codex/forge/companies/[active_company] checkout -b main --track origin/main
git -C ~/.codex/forge/companies/[active_company] pull --rebase origin main
```
...
✅ Company knowledge repo initialised and pushed.
   ...
   Remote: [url]
   Branch: main
   Location: ~/.codex/forge/companies/[active_company]/
...
Pull-before-push convention:
   ...
   ($company-sync not yet available — use the following manually until it is:
    git -C ~/.codex/forge/companies/[active_company] pull && git -C ~/.codex/forge/companies/[active_company] push)
