...
Where a specific model or provider is named, record it as `[MDL-TBD — <component, provider, model and version>]` for `$write-ord` to resolve into an `MDL-NNN` row. See `standards/ai.md` § *Model dependency* for the register's columns — pinning, deprecation notice, fallback behaviour and re-evaluation trigger.
...
Where a regulatory regime is in play, name which one applies and why. `standards/ai.md` § *Australian adoptions and instruments* records that a purely domestic Australian system carries no mandatory AI-specific requirement classes — never import the EU AI Act's classes by default.
...
**The evaluation-set schema lives in `standards/ai.md` § *Evaluation set register*.** Read it rather than restating it — it mandates the scorer (including the calibration set, agreement statistic and minimum for an LLM-judge), the threshold, the **floor** on the worst single case, the prohibited-output row IDs, the re-run trigger and the owner. Two of its rules bind every review:
...
Record assumptions separately, using the `ASM-NNN` prefix from `standards/tables.md`:
...
Convert vague statements into testable **proposed** criteria in the declarative present with no modals, per `standards/language.md`. `$write-ac` owns the `AC-NNN` namespace, so write `[AC-TBD — <behaviour>]`:
...
15. No ID was minted from a namespace `standards/tables.md` assigns to another skill.
