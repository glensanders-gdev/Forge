...
Reviews feed `$write-prd`, `$write-ord` and `$write-ac`, so they borrow those namespaces rather than minting their own. Read `standards/ai.md` and `standards/tables.md` before writing any register.
...
- Write criteria in the declarative present with no modals, per `standards/language.md`.
