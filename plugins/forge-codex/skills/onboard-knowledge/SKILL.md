...
5. If skipped: note that `~/.codex/forge/knowledge/company/style-guide.md` is a placeholder — remind user to fill it in before running `$check-style`.
...
- Run check-style to validate any deliverable against the style guide
```
...
- `check-style` — validates deliverables once `style-guide.md` is populated
