...
Session state is held per **stream** of work — see [STREAMS.md](../handoff/STREAMS.md). Debrief
...
2. **Resolve the stream** — per the resolution table in [STREAMS.md](../handoff/STREAMS.md). With more than one Active
   stream and no slug given, ask which one this session's work belongs to.
3. **Update `docs/handoffs/<slug>.md`** — overwrite using the stream-file template in [STREAMS.md](../handoff/STREAMS.md),
...
8. **Record usage evidence** — update the current phase record in `docs/tokens/[feature-name].md`
   using [TOKEN-RECORDING.md](../token-report/TOKEN-RECORDING.md). Use an exact Codex usage export
   or user-provided measurement when available. Otherwise record `Source: unavailable` and leave
   token counts blank; `ccusage` is Claude Code-only and must not be used for Codex sessions.
...
| Legacy single-document `docs/HANDOFF.md` | Migrate per [STREAMS.md](../handoff/STREAMS.md), confirming the slug, then proceed. |
