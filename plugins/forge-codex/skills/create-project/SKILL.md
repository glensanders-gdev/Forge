...
1. **Assign Project ID** — read `~/.codex/forge/registry.md`, get next PROJ number, increment counter. Note the ID (e.g. `PROJ-003`) for use throughout.
2. Read `~/.codex/forge/preferences.md` — confirm remote platform and username are set. If not, ask before proceeding.
3. Read `~/.codex/forge/ideas/active/[idea-name]$idea.md` — extract:
...
- Add idea reference to Knowledge References table:
  ```markdown
  | Idea Origin | `~/.codex/forge/ideas/active/[idea-name]$idea.md` |
  |             | `~/.codex/forge/ideas/active/[idea-name]/diagram.mmd` |
  ```
...
1. Update `AGENTS.md` in the new repo — add to the project header:
   ```markdown
   **Project ID:** PROJ-NNN
   **Origin:** IDEA-NNN ([idea name])
   ```

2. Update `~/.codex/forge/ideas/active/[idea-name]$idea.md` — add linked project:
   ```markdown
   **Project:** PROJ-NNN ([repo name])
   ```

3. Update `~/.codex/forge/registry.md`:
...
Update `~/.codex/forge/ideas/active/[idea-name]$idea.md`:
...
Before writing the PRD, run grill-me to validate the idea
scope against your systems and processes.
```

**2. Priority prompt:**
Read `~/.codex/forge/priorities.md` and display the current ranked list. Ask:
```
...
On confirmation, update `~/.codex/forge/priorities.md`.
...
- Always check `~/.codex/forge/preferences.md` before asking about platform or visibility
- If remote CLI tools are unavailable, provide manual steps — never fail silently
- The idea document is referenced, never copied — it stays in `~/.codex/forge/ideas/active/`
...
Before Step 6 (Create Remote and Push), check `~/.codex/forge/preferences.md` for `cli_available` setting. If not set, test by running `gh --version` or `glab --version`.
...
| Condition | Behaviour |
|-----------|-----------|
| `preferences.md` missing or incomplete | Stop. "preferences.md is not configured. Open ~/.codex/forge/preferences.md and fill in remote platform and username before running $create-project." |
