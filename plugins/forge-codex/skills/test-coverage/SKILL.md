...
- Read `AGENTS.md` — check for `coverage-threshold`, test conventions, or project-specific
  test file naming patterns (e.g. `*.spec.ts`, `*_test.go`)
- Read `~/.codex/forge/companies/[active_company]/tools.md` then `~/.codex/forge/tools/global.md` —
...
naming. Read `AGENTS.md` for the project file naming convention before writing.
...
After `$test-coverage` completes, update `~/.codex/forge/preferences.md`:
```
