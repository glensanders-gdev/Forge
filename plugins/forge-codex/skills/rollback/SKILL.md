...
Read `.codex/forge/deploy.md` — extract rollback command.

If no rollback command defined:
```
⛔ No rollback command configured in .codex/forge/deploy.md.
...
3. Run health check if URL configured in `.codex/forge/deploy.md`
...
2. Fix the issue and re-run deploy when ready
```
...
Update `~/.codex/forge/pi/[current-pi]/plan.md` — mark the affected release:
