...
Live Jira API integration — fetch, analyse, comment, transition, and search Jira tickets without leaving Codex.
...
**Option A — MCP server (recommended):** Add a Jira MCP server to Codex's MCP config. Run `$jira setup` for guided setup.
