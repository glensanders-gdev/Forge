...
2. **If MCP:** Show the MCP server config snippet to add to Codex's MCP settings.
   Config file location:
   - macOS: `~/Library/Application Support/Codex/claude_desktop_config.json`
   - Windows: `%APPDATA%\Codex\claude_desktop_config.json`
   Instruct the user to restart Codex after adding the server.
...
3. **If env vars:** Ask for `JIRA_URL`, then instruct the user to set `JIRA_EMAIL` and
   ...
   Store `JIRA_URL` in `~/.codex/forge/preferences.md`:
   ```
