...
Deploy the current project. Executes the deployment pipeline defined in `.codex/forge/deploy.md`. Requires a confirmed Go/No Go before proceeding. Single project only — for full PI releases use `deploy-pi`.

## Pre-Flight Checks

### 1. Read Deploy Config

Read `.codex/forge/deploy.md` — extract:
- Staging config (if present)
- Production config
- Health check URL
- Rollback command

**Cross-check with company config** — read `~/.codex/forge/companies/[active_company]/config.md` (if set):
- `deployment_chain` — compare to the environments in `.codex/forge/deploy.md`. If the chains differ, flag:
  ```
...
If `.codex/forge/deploy.md` is missing:
```
⛔ No deploy configuration found.

Create .codex/forge/deploy.md before deploying. A template is available in the Forge project template.
```
Stop.

**Rollback command validation:** If no rollback command is defined in `.codex/forge/deploy.md`, warn before proceeding:
```
⚠️ No rollback command configured.

If this deployment fails, rollback will require manual intervention.
It is strongly recommended to configure a rollback command in .codex/forge/deploy.md before deploying.
...
- If file missing → stop:
  ...
  go-nogo is a mandatory gate before deployment.
  Run it now to prepare the brief and record the GO decision,
  then re-run deploy.
  ```
- If decision is `NO-GO` → stop: "Go/No Go decision was NO-GO on [date]. Deployment blocked. Run `go-nogo` again to re-evaluate."
...
Suggest: "Now that this feature is deployed, consider a Post Implementation Review: `pir [PROJ-NNN]`. Did it achieve its stated goals in production?"
...
If `ROLLBACK`:
- Run rollback command from `.codex/forge/deploy.md`
- Verify health check
...
If no deployment command is defined in `.codex/forge/deploy.md`:
...
| Condition | Behaviour |
|-----------|-----------|
| `.codex/forge/deploy.md` missing | Stop. Provide template instructions. |
| Go/No Go record missing | Stop. Direct to `go-nogo`. |
