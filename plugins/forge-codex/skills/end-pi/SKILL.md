...
Formally close a Product Iteration. Produces a delivery summary, captures lessons learned, updates carry-forwards, and archives the PI plan. Mirrors the `$end-sprint` pattern at PI scale.

## When to Use

- The final release of the PI has been approved via `$go-nogo`
- User explicitly runs `end-pi`
...
Read `~/.codex/forge/pi/[current-pi]/plan.md`:
- Confirm all three releases have been closed (Go/No Go completed)
- If any release is still open, warn: "Release R[N] has not been closed. Run `go-nogo` before closing the PI."
- Identify carry-forward features

Read `~/.codex/forge/sprints/calendar.md` — confirm all PI sprints are marked Closed.
...
Append to `~/.codex/forge/pi/[current-pi]/plan.md`:
...
Append a delivery summary to `~/.codex/forge/pi/[current-pi]/stakeholder.md`:
...
Mark the PI as Closed in the plan header. Update `~/.codex/forge/sprints/calendar.md` — mark all PI sprints as archived.

### 8. Update Priorities

Read `~/.codex/forge/priorities.md` — remove delivered features, flag carried-forward features for re-ranking in the next PI.
...
- Update ~/.codex/forge/priorities.md ranking for carried-forward features
```
...
At PI end, read `~/.codex/forge/tokens/ledger.md` and filter for features in this PI:
