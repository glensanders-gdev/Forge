...
> **Company-aware:** When `active_company` is set in `~/.codex/forge/preferences.md` (configured by `$company-add`), publish config is read from `~/.codex/forge/companies/[active_company]/knowledge/publish/confluence.md` and wiki content is sourced from the company knowledge directory instead of the global paths below.
...
1. **Check config exists** — look for `~/.codex/forge/knowledge/publish/confluence.md`
...
### Ideas
Ideas are published directly from `~/.codex/forge/ideas/active/` — no knowledge base tier.
...
Archived ideas (`~/.codex/forge/ideas/archived/`) are not published — they remain local only.
...
4. **Log result** — append to `~/.codex/forge/knowledge/publish/publish-log.md`:
   ```
...
Failed pages are tracked in `~/.codex/forge/knowledge/publish/publish-log.md` — a retry queue
...
To set up:
1. Copy ~/.codex/forge/knowledge/publish/confluence.example.md
   to ~/.codex/forge/knowledge/publish/confluence.md
