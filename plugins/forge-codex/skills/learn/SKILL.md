...
Capture a pattern from the current session into `~/.codex/forge/instincts/` so it accumulates over time and can eventually be promoted into a formal Forge skill.
...
Read `~/.codex/forge/instincts/registry.md` and all existing instinct files. Look for:
...
1. Read `~/.codex/forge/instincts/registry.md` — get next instinct number
2. Generate a slug from the observation (3–5 words, hyphenated, lowercase)
3. Create `~/.codex/forge/instincts/instinct-NNN-[slug].md` from `_template.md`
...
File: ~/.codex/forge/instincts/instinct-NNN-[slug].md

This instinct will reach Medium confidence after 2 more observations
and High confidence after 4 more. Run evolve when ready to
