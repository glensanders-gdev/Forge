---
name: "sync-company"
description: "Sync the local company knowledge directory with the team GitHub remote. Pull latest changes then push local changes. Supports --pull-only and --push-only flags. Use when user runs $sync-company, after $ingest to share new wiki articles, or at the start of a session to get latest team knowledge."
metadata:
  category: company
...
Synchronise the local company knowledge directory at `~/.codex/forge/companies/[active_company]/`
...
```
$sync-company              ← pull then push (default)
$sync-company --pull-only  ← get latest without sharing local changes
$sync-company --push-only  ← share local changes without pulling first
```

`$ingest` offers to run `$sync-company --push-only` automatically after compiling new
...
Check `~/.codex/forge/companies/[active_company]/.git` exists. If not:
```
...
Run `git -C ~/.codex/forge/companies/[active_company] remote get-url origin`.
If no remote:
```
...
Read `~/.codex/forge/companies/[active_company]/config.md`. Extract:
...
```bash
git -C ~/.codex/forge/companies/[active_company] fetch [git_remote]
git -C ~/.codex/forge/companies/[active_company] pull --rebase [git_remote] [git_branch]
```
...
⚠️ Merge conflicts detected.
   ...
   2. Mark resolved:
      git -C ~/.codex/forge/companies/[active_company] add [file]
   3. Complete the rebase:
      git -C ~/.codex/forge/companies/[active_company] rebase --continue
   4. Re-run $sync-company --push-only
...
```bash
git -C ~/.codex/forge/companies/[active_company] status --porcelain
```
...
On `SYNC`:
```bash
git -C ~/.codex/forge/companies/[active_company] add .
git -C ~/.codex/forge/companies/[active_company] commit -m "chore: sync knowledge base [YYYY-MM-DD]"
git -C ~/.codex/forge/companies/[active_company] push [git_remote] [git_branch]
```
...
❌ Push failed. Output:
   ...
   Common causes:
   - Remote has changes you haven't pulled: run $sync-company --pull-only first
...
```
✅ Sync complete — ~/.codex/forge/companies/[active_company]/
