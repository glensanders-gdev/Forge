...
Review a document, PRD, report, or any written output against `~/.codex/forge/knowledge/company/style-guide.md`. Produces a findings report and a pass/fail gate.
...
Before reviewing, read `~/.codex/forge/knowledge/company/style-guide.md`.

If the file is a placeholder (no sections filled in):
```
⚠️ Style guide not yet populated.

Fill in ~/.codex/forge/knowledge/company/style-guide.md before running $style-check.
Run knowledge-onboard to set it up guided, or edit the file directly.
```
...
**Reviewed against:** ~/.codex/forge/knowledge/company/style-guide.md
**Date:** YYYY-MM-DD
...
- `write-article` — recommends `$style-check` on completion
- `write-prd` — recommends `$style-check` on completion
- `knowledge-onboard` — populates `style-guide.md` that this skill reads
- `qa-plan` — include a style-check step for any externally shared deliverable

## Related

- `~/.codex/forge/knowledge/company/style-guide.md` — canonical **Company Style Guide** this skill reads
...
| `style-guide.md` missing entirely | "Style guide not found at expected path. Run knowledge-onboard to set it up." |
