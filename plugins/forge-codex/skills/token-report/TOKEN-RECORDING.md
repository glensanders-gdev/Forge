...
Codex does not expose exact token counts. Estimate as follows:
...
Increment the session count each time the phase continues in a new Codex session. A single uninterrupted session = 1. A phase resumed after closing and reopening Codex = 2, etc.
