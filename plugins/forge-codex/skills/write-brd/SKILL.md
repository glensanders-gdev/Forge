...
2. **Read every source in full** — a `$front-gate` Request Brief, `~/.codex/forge/ideas/active/*$idea.md`,
...
2. **Write every section of the anatomy**, in the standard's order, to
   ...
   assumptions and dependencies `ASM-NNN` / `DEP-NNN` per `standards/tables.md`. Constraints carry **no local
