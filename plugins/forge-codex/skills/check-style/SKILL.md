...
Fill in ~/.codex/forge/knowledge/company/style-guide.md before running $check-style.
Run onboard-knowledge to set it up guided, or edit the file directly.
```
...
- `write-article` — recommends `$check-style` on completion
- `write-prd` — recommends `$check-style` on completion
- `onboard-knowledge` — populates `style-guide.md` that this skill reads
- `qa-plan` — include a check-style step for any externally shared deliverable
...
| `style-guide.md` missing entirely | "Style guide not found at expected path. Run onboard-knowledge to set it up." |
