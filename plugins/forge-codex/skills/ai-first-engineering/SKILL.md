...
*In practice:* `$review-diff` checks against ADRs, `CONTEXT.md`, and `CODING-STANDARDS.md` — not style. `$check-pii` covers data integrity. `$go-nogo` covers rollout safety.
...
*In practice:* `$review-diff` uses this checklist. `$check-pii` covers data integrity specifically. `$go-nogo` is the final rollout safety gate.
...
- Never run `$build` on a vaguely specified PRD, and never deploy without a rollout safety gate (`$go-nogo`).
