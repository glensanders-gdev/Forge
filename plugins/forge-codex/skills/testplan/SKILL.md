...
3. Read relevant `~/.codex/forge/knowledge/systems/*/known-issues.md` — known constraints affect what can be tested.
...
11. Suggest: "Run `tdd` to implement automated tests, `qa-plan` for manual verification."
...
2. Read `docs/CONTEXT.md` — test names must use domain language.
3. Read relevant `~/.codex/forge/knowledge/systems/*/known-issues.md`.
...
```
Feature:      $write-prd → $testplan → $qa-plan → $qa-report → $approve
Operational:  $write-ord → $testplan --operational → (release verification)
```

Precedes: `$tdd` (automated test implementation), `$qa-plan` (manual verification plan)
Follows: `$write-prd` (feature mode), `$write-ord` or `$write-reqs` (operational mode)

**Known downstream limitation.** `$qa-plan`, `$qa-report` and `$go-nogo` are feature-scoped and read
the active PRD — none of them reads `docs/testplan-operational.md` today. Operational manual TCs and
the Inspection bucket therefore need a human to carry them into the release gate. State this when
handing off, so the handoff is carried by a person rather than silently lost at the next skill.
