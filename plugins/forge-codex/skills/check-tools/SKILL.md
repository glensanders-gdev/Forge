...
```
$check-tools                     ← check all categories
$check-tools security-scanner    ← check one category only
$check-tools --company [name]    ← check against a specific company (overrides active_company)
```
...
1. Read `~/.codex/forge/tools/global.md` — the global tool list
2. If `active_company` is set in `~/.codex/forge/preferences.md` (or `--company` flag provided):
   - Read `~/.codex/forge/companies/[name]/tools.md`
...
| Condition | Behaviour |
|-----------|-----------|
| `~/.codex/forge/tools/global.md` missing | Note "Global tools registry not found. Run `bash install.sh` to scaffold it." Stop. |
| Company tools file missing | Note "No company tools configured for [name]. Run `$add-tool --company [name]` to set up." Proceed with global registry only. |
| check-command times out | Record as "unknown" — note in output |
| No tools registered | Note "No tools registered in the global registry." Suggest `$add-tool`. |
