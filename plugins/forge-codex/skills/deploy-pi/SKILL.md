...
Deploy all projects in the current PI release in the order defined by their `deploy-order` field in `.codex/forge/deploy.md`. A single project failure stops the entire sequence — the human decides how to proceed.

## Pre-Flight Checks

### 1. Identify Current PI Release

Read `~/.codex/forge/pi/[current-pi]/plan.md` — identify the release being deployed (R1, R2, or R3).

### 2. Build Deployment Sequence

For each active project in the PI:
- Read `.codex/forge/deploy.md`
- Extract `deploy-order` field
- Sort projects by `deploy-order` (ascending — lowest first)

If any project has no `.codex/forge/deploy.md`:
```
...
Update each project's entry in `~/.codex/forge/pi/[current-pi]/plan.md` individually, regardless of outcome:
...
**Full success only:** Update `~/.codex/forge/pi/[current-pi]/stakeholder.md` — mark all deployed features as `✅ Delivered — [date]`.
...
| Condition | Behaviour |
|-----------|-----------|
| No active PI plan | Stop. "No active PI plan found. Run `piplan` first." |
...
| Go/No Go missing for release | Stop. Direct to `go-nogo`. |
