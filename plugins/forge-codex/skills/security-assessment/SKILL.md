...
Read `~/.codex/forge/companies/[active_company]/config.md` (if set) for `compliance_tier`
...
1. Read `~/.codex/forge/companies/[active_company]/tools.md` (if company is set) — company entries take priority
2. Read `~/.codex/forge/tools/global.md` — global fallback
...
- `AGENTS.md` (project root) — architecture overview, tech stack, known constraints
...
| `AGENTS.md` missing | Note missing — proceed with directory structure inference only |
