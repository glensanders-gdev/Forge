...
- `AGENTS.md` — if present, Forge may already be set up. Confirm with user before proceeding.
...
1. **Copy Forge project template files** — only files that don't already exist:
   - `AGENTS.md` — fill in project context from Phase 1 findings
...
5. **Fill `AGENTS.md`** — from Phase 1 findings:
...
- If `AGENTS.md` already exists, stop and ask: "Forge appears to already be set up. Do you want to re-onboard or update the existing setup?"
...
1. Read `~/.codex/forge/registry.md` — get next PROJ number, increment counter
2. Add to `AGENTS.md`:
   ```markdown
   **Project ID:** PROJ-NNN
   **Origin:** None (onboarded existing project)
   ```
3. Add to `~/.codex/forge/registry.md` Project Registry:
   ```markdown
...
| Condition | Behaviour |
|-----------|-----------|
| `AGENTS.md` already exists | Stop and ask whether to re-onboard or update — Forge may already be set up. |
