...
1. **Read the sprint calendar** — `~/.codex/forge/sprints/calendar.md`
   ...
   - If no current sprint exists, ask the user:
     ...
     - If generate: read `sprint_length_weeks` from `~/.codex/forge/companies/[active_company]/config.md` (default 2 if not set or no company config). Propose start = today, end = today + (sprint_length_weeks × 7 − 1) days. Confirm before adding to calendar.
...
4. **Surface global backlog items** — read `~/.codex/forge/backlog.md`:
...
6. **Check company config** — read `~/.codex/forge/companies/[active_company]/config.md` (if set):
...
```
$start-sprint → $standup (daily) → $end-sprint → $debrief
```

Precedes: `$standup` (daily rhythm during the sprint)
Follows: `$sprintplan`, `$replan-pi`, or manual sprint open
...
_To be completed at sprint close — run `end-sprint`_
```
...
- Suggest running `standup` at the start of each session during the sprint.

## Sprint Capacity Check

After sprint goals are confirmed and tickets are assigned, check capacity:

1. Read `sprint-capacity-points` and `sprint-capacity-tokens` from `~/.codex/forge/preferences.md` (defaults: 20pts, 400k tokens if not set).
...
After the capacity check, check `context-health-last-run` in `~/.codex/forge/preferences.md`.
...
| Condition | Behaviour |
|-----------|-----------|
| No matching entry in `~/.codex/forge/sprints/calendar.md` | Resolve the calendar first — offer to generate dates or sync; never write a record without one. |
