...
it.** See `~/.codex/forge/rules/requirements/language.md` § *Demand, not design*. The ORD precedes
...
`~/.codex/forge/rules/requirements/tables.md`. A KPP that cannot reach at least `Provisional` inside the
...
`~/.codex/forge/rules/requirements/tables.md` and are authoritative there. This template shows where each
