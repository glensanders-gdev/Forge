...
here are used by `$security-assessment`, `$review-performance`, `$build` pre-flight, and
...
```
$add-tool                          ← add to global registry (~/.codex/forge/tools/global.md)
$add-tool --company [name]         ← add to company registry (~/.codex/forge/companies/[name]/tools.md)
$add-tool --company [name] trivy prohibited   ← quick-add: mark an existing global tool as prohibited
```
...
- If `--company [name]` flag: target `~/.codex/forge/companies/[name]/tools.md`
  ...
  - Verify the company directory exists — if not, suggest running `$add-company [name]` first.
- Otherwise: target `~/.codex/forge/tools/global.md`

### Step 2 — Quick-add (optional)

If the user provides `name` and `status` inline (e.g. `$add-tool --company acme trivy prohibited`):
...
✅ [tool-name] registered in [global / [company]] tools registry.
   ...
   Category: [category]
   Run $check-tools to verify it is installed on this machine.
```
...
| Condition | Behaviour |
|-----------|-----------|
| Company directory doesn't exist | Stop — suggest `$add-company [name]` first |
...
- `$check-tools` — reads the same **Forge Tools Registry** (`~/.codex/forge/tools/global.md`) to verify installations
- `$security-assessment`, `$review-performance`, `$build` — query the registry to discover available tools
...
- Never auto-run check-command during registration — `$check-tools` does that
