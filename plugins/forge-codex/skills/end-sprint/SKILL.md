...
Triggered when the human runs `end-sprint` or picks up the HITL finalise ticket.
...
9. **Update the sprint's stream handoff** — resolve the stream per
   ...
   - Next action: "Run `start-sprint` to open Sprint-NN+1"
...
11. Prompt (do not trigger): "Sprint-NN closed. Run `start-sprint` when you're ready to open Sprint-NN+1."
