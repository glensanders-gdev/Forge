...
- User explicitly runs `/user:rollback-pi`
...
Read `~/.claude/pi/[current-pi]/plan.md` — confirm which release is being rolled back.

### 2. Build Reverse Deployment Sequence

Read each project's `.claude/deploy.md` — extract `deploy-order`. Sort in **descending** order (highest deploy-order first = last deployed first).
...
Check each project's `.claude/deploy.md` for a rollback command. If any project is missing one:
```
...
2. Fix the issue and re-run /user:deploy-pi when ready
3. Consider running /user:update-readme to note the rollback in version history
```
...
Update `~/.claude/pi/[current-pi]/plan.md`:
