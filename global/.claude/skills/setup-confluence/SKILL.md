...
> **Company-aware:** When `active_company` is set in `~/.claude/preferences.md` (configured by `/add-company`), `confluence.md` is written to `~/.claude/companies/[active_company]/knowledge/publish/` instead of `~/.claude/knowledge/publish/`.
