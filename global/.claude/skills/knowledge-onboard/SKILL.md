...
Guided end-to-end setup of `~/.claude/knowledge/` for a new employer. Runs four stages in strict sequence — each stage is HITL-gated before the next begins. Nothing is written without your confirmation.
...
5. If skipped: note that `~/.claude/knowledge/company/style-guide.md` is a placeholder — remind user to fill it in before running `/style-check`.
6. Draft the completed `style-guide.md` and present for HITL review.
7. On confirmation, write to `~/.claude/knowledge/company/style-guide.md`.
...
4. On confirmation, append to `~/.claude/knowledge/company/acronyms.md` in alphabetical order.
...
4. On confirmation, append to `~/.claude/knowledge/company/context.md`.
...
3. Use `/user:summarise-system` logic to draft `overview.md`:
...
Written:
- ~/.claude/knowledge/company/style-guide.md
- ~/.claude/knowledge/company/acronyms.md (N entries)
- ~/.claude/knowledge/company/context.md (N terms)
- ~/.claude/knowledge/systems/[name]/ (N systems)

Next steps:
- Run /user:knowledge-health to check coverage
- Add systems incrementally as you encounter them using /user:add-system
- Fill in any placeholder sections left during this session
- Run /user:style-check to validate any deliverable against the style guide
```
...
- `/user:summarise-system` — Stage 4 uses its extraction logic for system overviews
- `/user:add-term` — used for any terms discovered mid-session that need capturing immediately
- `/user:knowledge-health` — run after completion to check coverage and flag gaps
- `/user:style-check` — validates deliverables once `style-guide.md` is populated
- `CLAUDE.md` (project-level) — remind user to reference newly documented systems in project CLAUDE.md files
