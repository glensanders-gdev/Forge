...
5. If skipped: note that `~/.claude/knowledge/company/style-guide.md` is a placeholder — remind user to fill it in before running `/check-style`.
...
- Run /user:check-style to validate any deliverable against the style guide
```
...
- `/user:check-style` — validates deliverables once `style-guide.md` is populated
