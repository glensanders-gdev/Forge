...
name: update-company
category: company
description: Update an existing company Forge install — re-run grilling topics to reconfigure operational settings, or refresh bundled skills from the global install. Use when company config needs updating after /add-company, or after a Forge upgrade to push new skill versions to the company repo.
...
```
/update-company --reconfigure    ← re-run selected grilling topics from /add-company
/update-company --update-skills  ← refresh 17 bundled skills from ~/.claude/
/update-company --all            ← both, in order: reconfigure first, then refresh skills
```
...
Confirm target directory exists: `~/.claude/companies/[active_company]/config.md`.
If missing:
```
❌ Company config not found at ~/.claude/companies/[active_company]/config.md.
   Directory may have been moved or deleted. Re-run /add-company [name] to rebuild.
```
Exit.
...
Re-run any of the 8 grilling topics from `/add-company` against the existing `config.md`.
...
For each selected topic, present the same grilling questions as in `/add-company`.
...
[Questions from /add-company Topic 1]
```
...
```
✅ Config updated — ~/.claude/companies/[active_company]/config.md

   Topics updated: [list]
   Fields changed: N

   Skills that read this config:
   /standup, /start-sprint, /go-nogo, /deploy, /check-pii, /build, /dashboard-tokens

   Run /sync-company to share the updated config with the team.
```
...
`~/.claude/skills/` install. Use after a Forge upgrade to propagate fixes and
...
- `~/.claude/companies/[active_company]/.claude/skills/[skill]/SKILL.md` (current)
- `~/.claude/skills/[skill]/SKILL.md` (available)
...
Skill version check — 17 bundled skills
  ...
  check-pii          1.0.0      1.1.0      ⬆ update available
  ...

Updates available: N skills
Already current:   N skills
Missing from ~/.claude/ (cannot update): N skills
...
1. Copy `~/.claude/skills/[skill]/SKILL.md` → `~/.claude/companies/[active_company]/.claude/skills/[skill]/SKILL.md`
2. Copy `~/.claude/commands/[skill].md` → `~/.claude/companies/[active_company]/.claude/commands/[skill].md`

For skills missing from `~/.claude/` (not installed): note and skip — do not remove the existing bundled copy.

### Step 3 — Confirm

```
✅ Skills refreshed — ~/.claude/companies/[active_company]/.claude/

   Updated: [N skills — list]
   Skipped (already current): N skills
   Skipped (not in ~/.claude/): [list if any]

   Run /sync-company to push updated skills to the team repo.
```
...
| Condition | Behaviour |
|-----------|-----------|
| `active_company` not set | Exit with /add-company message |
| Config file missing | Exit with rebuild instruction |
| Skill missing from `~/.claude/` | Skip it, note in output — do not block |
