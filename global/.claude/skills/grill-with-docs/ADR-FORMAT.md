...
- Reference the ADR filename in `docs/DEVLOG.md` under "Decisions Made" for the session
- Reference it in the relevant kanban ticket or PRD Implementation Decisions section
- Superseded ADRs are never deleted — mark status as `superseded by ADR-NNNN`
- The `write-adr` skill (`/user:write-adr`) can be used to create ADRs explicitly outside of a grill session
