...
1. Read `docs/kanban.md` — identify current sprint tickets (In Progress and Backlog). If none found: "No sprint tickets found in kanban.md. Run `/user:start-sprint` to open a sprint first."
...
Once the ticket's tests are green, run `/user:review-diff` on **this ticket's diff** (the files written during this ticket) — the two-axis review: Spec (does the diff fulfil the ticket's requirement) and Standards (project docs + smell baseline). This is AFK and advisory — it does not auto-fix.
...
| Condition | Behaviour |
|-----------|-----------|
| No kanban found | "No kanban.md found. Run `/user:start-sprint` to open a sprint first." |
| No sprint tickets | "No sprint tickets found. Add tickets via `/user:add-backlog-item` or `/user:write-prd`." |
