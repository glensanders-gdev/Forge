...
- User runs `/user:evolve` explicitly
...
Read `~/.claude/instincts/registry.md` and all instinct files in `~/.claude/instincts/`.
...
2. Suggest: "Running `/user:write-a-skill` now — the instinct will be the brief."
...
Move the instinct file to `~/.claude/instincts/archived/`. Update registry — mark as Archived with a note.
...
Run /user:learn during sessions to accumulate observations.
```
...
| Registry missing | "No instincts registry found. Run /user:learn to start capturing patterns." |
