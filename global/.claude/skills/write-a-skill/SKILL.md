...
- [ ] **Git tag created** — after merging to `main`, tag the release: `git tag v[forge_version] <main-sha> && git push origin v[forge_version]`. Tag must match `forge_version` in `manifest.json` exactly (e.g. `v3.5.0`). This is what keeps devices in sync — `/forge-update` compares version strings, but tags anchor those strings to a specific commit.
