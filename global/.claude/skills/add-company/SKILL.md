...
category: company
standalone: false
description: Set up a company-specific file structure outside the Forge repo at ~/.claude/companies/[name]/. Grills the user on company operational config — sprint cadence, team locations, public holidays, freeze periods, compliance tier, external approval gates, deployment environments, AI usage policy, and tools policy. Scaffolds knowledge, projects, rules, and tools sections mirroring the global structure. Company data is never committed to the Forge GitHub. Use when user runs /add-company [name] or sets up Forge for a new company install.
---
...
If the user runs `/add-company [name] --quick`:
...
5. End with:
   ...
   Run /add-company [name] (without --quick) at any time to redo the full grilling session.
   ```
...
⚠️ Existing Forge data found at the global paths:
   ...
   After /add-company, all skills will resolve paths to the new company
...
deadlines, how /end-sprint calculates carry-over, and how /go-nogo times
release gates.
```
...
/build pre-flight, /security-assessment, and /update-dependencies query. Full
tool details (check-command, install-hint, usage) are added later via
/add-tool --company [name] — here we capture policy intent only.
```
...
`# TODO: run /add-tool --company [name] to add full details` comment per entry.
...
## Team Locations
# Used by /standup and /start-sprint to flag public holidays.
...
## Git Remote
# Used by /sync-company and /update-company.
...
# perform pushes manually — /sync-company will stage and commit but remind you to open a PR.
git_remote: origin
git_branch: main
```
...
- `knowledge/legal/Wiki/_index.md` — legal wiki index stub (sensitive — note that contents may be legally privileged; run `/check-pii` before sharing or publishing)
...
- `tools.md` — pre-populate with skeleton entries for any prohibited/required/approved tools captured in Topic 8; each entry flagged with `# TODO: run /add-tool --company [name] to complete details`; if none captured, create an empty `tools.md` with the standard header (see `/add-tool` appendix template)
...
Copy the following 17 skills from the user's global `~/.claude/` install into the company repo at the time `/add-company` runs. Each skill gets two files:
...
| `check-style` | Validate docs against style guide |
| `check-pii` | Check for PII exposure |
| `sync-company` | Push/pull to company git remote |
...
| `onboard-knowledge` | Initial company knowledge setup |
...
| `/lookup` | Search the knowledge base |
| `/sync-company` | Push/pull knowledge updates with the team |
| `/check-style` | Validate a document against the company style guide |
| `/check-pii` | Check content for PII before sharing |
...
Run `/init-forge` silently (suppress its completion output — this skill produces its own).
...
Skills that read your new config:
  ...
  /start-sprint    → flags public holidays falling within the sprint
...
Next steps:
  ...
  7. Run /add-tool --company [name] to complete tool details for entries registered during setup
  8. Run /check-tools to verify required tools are installed
  ...
  10. Rename technology1–technology8 to your actual domain names, e.g.:
      ...
      Run /update-company to sync the renamed sub-categories with the team.
```
