...
<!--forge-only-->> **Company-aware:** When `active_company` is set in `~/.claude/preferences.md` (configured by `/add-company`), all Raw/ and Wiki/ paths resolve under `~/.claude/companies/[active_company]/knowledge/` instead of `~/.claude/knowledge/`.

<!--/forge-only-->Compile unprocessed source material into the Wiki. Raw is always the origin — all intake
...
<!--forge-only-->The knowledge root is `~/.claude/knowledge/`, or a `systems/[name]/` or `projects/[name]/`
folder beneath it. `/add-system` and `/add-project` scaffold the three tiers.
<!--/forge-only-->Where no knowledge root exists yet, create `knowledge/Raw/` and `knowledge/Wiki/` at the
...
1. Collect the candidate spaces<!--forge-only-->. Read `~/.claude/registry.md` and collect all active projects (PROJ-NNN rows with status Active)<!--/forge-only-->.
...
5. **Check for cross-system scope** — if the concept requires two or more system names to
   explain, it belongs in the top-level `Wiki/`.<!--forge-only--> Check `cross_system_gate` in
   ...
   - `confirmed` → proceed without confirmation<!--/forge-only-->
...
12. **Present a summary**:
    ...
    - Items remaining in the inbox<!--forge-only-->
    - Reminder to run `/knowledge-health` if failure count > 0<!--/forge-only-->
...
| Concept spans 2+ systems | It belongs in the top-level `Wiki/`<!--forge-only--> — confirm via `cross_system_gate` before creating<!--/forge-only-->. |
...
| Log line names a parenthetical note, not a file | Skip it — the compile had no source file by design. |<!--forge-only-->
| `active_company` set with a git remote | Offer `/sync-company --push-only` after the run — never push automatically. |<!--/forge-only-->
