...
*In practice:* `/review-diff` checks against ADRs, `CONTEXT.md`, and `CODING-STANDARDS.md` — not style. `/check-pii` covers data integrity.<!--forge-only--> `/go-nogo` covers rollout safety.<!--/forge-only-->
...
*In practice:* `/review-diff` uses this checklist. `/check-pii` covers data integrity specifically.<!--forge-only--> `/go-nogo` is the final rollout safety gate.<!--/forge-only-->
...
<!--forge-only-->| `/go-nogo` | Rollout safety gate — human decides |<!--/forge-only-->
...
- Never run `/build` on a vaguely specified PRD, and never deploy without a rollout safety gate<!--forge-only--> (`/go-nogo`)<!--/forge-only-->.
