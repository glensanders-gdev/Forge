...
```
- [ ] [AFK] #N Draft end-sprint record `end-sprint --draft`
```
...
6. Add a HITL ticket to `docs/kanban.md`:
   ```
   - [ ] [HITL] #N+1 Finalise end-sprint record `blocked-by: #N`
   ```
...
Triggered when the human runs `/user:end-sprint` or picks up the HITL finalise ticket.
...
9. **Update the sprint's stream handoff** — resolve the stream per
   `~/.claude/skills/handoff/STREAMS.md`, then overwrite `docs/handoffs/<slug>.md` with sprint
   close state and update its row in the register at `docs/HANDOFF.md`:
   - Session type: Sprint Close
   - Current ticket: "Sprint-NN closed"
   - What just happened: sprint summary in one sentence
   - Next action: "Run `/user:start-sprint` to open Sprint-NN+1"
   ...
   - Where the sprint spanned several streams, update the one the sprint was run for and leave the
     others alone — `/debrief` sweeps the register, `/end-sprint` does not.
## Pipeline Position
...
Precedes: `/debrief` (session close), `/start-sprint` (next sprint)
...
11. Prompt (do not trigger): "Sprint-NN closed. Run `/user:start-sprint` when you're ready to open Sprint-NN+1."
...
- Do not auto-trigger `/start-sprint` — prompt only.

## Token Summary (Sprint)

At sprint end (HITL mode), build the sprint token summary from actuals:

1. Run `npx ccusage daily --since [start-sprint] --until [end-sprint] --json` — the measured sprint total.
2. Read `docs/tokens/[feature].md` for each feature worked this sprint and sum their recorded totals.
3. If the ccusage total and the sum of feature records differ materially (>20%), sessions were missed at `/debrief` — backfill the gap dates per `~/.claude/skills/token-report/TOKEN-RECORDING.md` before writing the summary.
