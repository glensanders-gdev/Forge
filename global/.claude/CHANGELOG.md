...
`~/.claude/companies/`, and a gated move that relocates a project folder between tiers. The
...
`<!--forge-only-->` fences. The standalone reader gets the three intake modes, the pipeline, the
...
the v4.7.0–v4.7.3 pull request, not by review.**

`plugins/forge-codex/` is generated by `tools/build-forge-codex.ps1` and its freshness is enforced by
the `parity` job. Its `upstream-manifest.json` read **4.6.4** while the tree was at 4.7.3, so the
first CI run on PR #68 failed with *"Codex plugin output is stale."*

**The consequence was the problem v4.7.0 exists to fix, one layer down.** A Codex user was running
the **pre-convergence** `write-ord` — the generic 11-column register with `Verification` and
`Delivery Agent` — against **post-convergence** review criteria, which is two standards disagreeing
about the same document. That is the exact failure v4.7.0 named and repaired for the Claude side.

### Changed — generated plugin output only

19 files, all emitted by the generator from `global/`. Nothing hand-authored.

- **`rules/requirements/reporting.md` reached the plugin for the first time.** It shipped in v4.7.0
  and was never propagated; the plugin carried only `language.md`, `tables.md`, `ai.md` and
  `README.md`. The adapted copy carries the v4.7.1 AS designations.
- **The `write-ord` 2.0.0 convergence** — the 707-line `REFERENCE.md` rewrite and the 309-line
  `SKILL.md` change.
- **A v4.7.0 cross-link change the plugin never received** — `write-reqs`'s Appendix B → Appendix A
  move, along with the `write-ac`, `testplan` and `review-ord` updates from v4.7.0 and v4.7.3.

Both version stamps now read 4.7.4. `test-forge-parity.ps1` passes: 113 shared skills, 114 Claude
commands, 115 Codex skills.

### Known — the review missed what the gate caught

The pre-push review of v4.7.0–v4.7.2 swept `global/` and `dist/` for residue of the removed register
columns and **never looked at `plugins/`**, despite the plugin's staleness having been flagged
earlier in the same session. **Three generated trees exist — `dist/`, `plugins/forge-codex/` and the
`review-*` extracts — and only two were checked.** The parity gate is doing work the review method
does not. This is the sixth occurrence of the class the v4.7.3 entry names, and the second in which
a mechanical check found what reading did not.
...
The pack assessed its BRD and ORD handoff gates for AI impact, a question carried across three
sessions and never asked: the AI work to date established *where an AI requirement goes* and never
whether the gates that **admit** and **release** the documents change. **Both gates hold** — no new
gate item, no new section, no change of position — because both are demand-side, and the demand side
is where an AI subject changes least. Everything AI changes is response-side and was routed there at
pack v1.6.

Four pack changes reach the generated extracts:

- **§5** — a tolerance over generated or learned output reaches Committed on its own evidence; the
  absence of an evaluation set is not a bar to it. `review-ord` now checks against this, so a
  reviewer will no longer read a missing evaluation set as grounds for Provisional.
- **OH-15 extended** — where the subject is generated or learned output, the requirement carries the
  obligation attaching to an **incorrect** answer, which is neither an adverse outcome nor a Rainy
  Day.
- **The BRD tolerance-or-figure table** gains a demand/response pair for an evaluation instrument,
  and its Existence test now states that a set which does not yet exist cannot carry a pass mark.
  Reaches `review-brd/CRITERIA.md` and `write-brd/STANDARD.md`.
- **Two §9 limitations** — E4's weaker yield where the subject is generated or learned output, and
  the effort figures deliberately not recomputed for it.

### Changed — generated extracts only

`review-ord/CRITERIA.md`, `review-brd/CRITERIA.md` and `write-brd/STANDARD.md`, regenerated by
`tools/build-review-criteria.py` from pack commit `d2f74eca4885` and stamped with it. **No skill
logic changed.** These files are generated — never hand-edit them; change the pack and regenerate.

### Known — a section-numbering disagreement between this repo and the pack

The pack's §3.8 Functional Suitability carries ISO/IEC 25010's three sub-characteristics —
Completeness, Correctness, Appropriateness. **`write-ord/REFERENCE.md` scaffolds §3.8 as Completeness
plus two ISO/IEC 25059 additions numbered 3.8.2 and 3.8.3**, which collide with Correctness and
Appropriateness, and `rules/requirements/ai.md`'s class map routes accuracy and fairness to §3.8.3 on
that basis.

Both readings are defensible — an ORD arguably carries Completeness only, Correctness being
functional content it declines — so **neither side was changed here.** The pack's new BRD row cites
§3.8 with no sub-characteristic rather than baking a contested number into teaching material.
**Resolve before either side cites the other.**
...
The committed extracts were stamped pack **v1.4**; the pack is at **v1.9**. Only whoever holds the
pack can see this — `build-review-criteria.py --check` skips where the pack is absent, so CI has
been passing over it. Regenerated against pack v1.9 (`719eecb78e35`).

### Changed — regenerated extracts

- **`review-ord` 2.0.0 → 2.1.0.** The substantive change: +201 lines of AI-component criteria.
  A phrasing-to-section map for learned or generated behaviour (25059 sub-characteristics landing in
  existing §3.x rows — no section added, none renumbered); the demand/response boundary for an
  evaluation threshold (the ORD names the **population**, the design response draws the set, picks
  the scorer and sets the pass mark, recorded at Appendix D); where a model or provider dependency
  lands and when it lands nowhere; and an EU AI Act Arts. 9–15 + 50 → ORD section map. §9 is renamed
  *Trade-offs, risk and dependencies*.
- **`review-brd` 2.0.0 → 2.0.1** and **`write-brd` 1.1.1 → 1.1.2** — **stamp only.** Both content
  hashes are byte-identical across v1.4→v1.9 (`397b4a4489d959ef`, `9d3e57310592cd19`); the BRD
  material did not move. Patch bumps because the directories changed and `/skill-health` compares
  directory mtime against the version's commit date.

### Known

- **The pack HEAD is untagged.** Earlier extracts stamped `(tag v1.4)`; v1.9 stamps a bare commit,
  so the extracts now cite a commit no tag resolves. Tag the pack at v1.9 and regenerate to restore
  the tagged reference.
- **`write-ord/REFERENCE.md` §9.2 holds the assumption register; the pack's anatomy puts it at
  Appendix B.** Pre-existing divergence, not introduced here, and untouched by this regeneration.
...
word for the third, even though `/add-company` has scaffolded `~/.claude/companies/[name]/.claude/skills/`
...
- A company skill lands under `~/.claude/companies/[active]/.claude/`, carries no `standalone:` key,
...
- The whole destination fork is fenced `<!--forge-only-->`. The published copy of the skill, if it
...
- Command stubs, the `/user:grill-me` row in the command reference, and the `origin:` frontmatter on
