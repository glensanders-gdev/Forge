...
**The companion is generated by script, not by the model.** `write-ord/scripts/llm_companion.py`
(standard-library Python) implements the rules file, and `/write-brd` runs the same copy. A model
asked to copy a thousand lines verbatim rewords some of them invisibly; the script copies by
