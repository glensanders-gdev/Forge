...
- `forge/project-template/CLAUDE.md` — split to ~186 lines, references 4 on-demand files
- `forge/project-template/.claude/CODING-STANDARDS.md` — extracted from CLAUDE.md (new)
- `forge/project-template/.claude/ERROR-HANDLING.md` — extracted from CLAUDE.md (new)
- `forge/project-template/.claude/SECURITY.md` — extracted from CLAUDE.md (new)
- `forge/project-template/.claude/TESTING.md` — extracted from CLAUDE.md (new)
...
1. **Run `/user:critic`** — the framework has grown significantly and a fresh P1/P2 sweep is warranted before the next major feature
...
4. **Backlog item:** cross-project prioritisation framework discussion (P3, `~/.claude/backlog.md`)
...
1. `/user:critic` — the framework hasn't had a fresh critique since v2.3.5 and is now at v2.3.7 with significant additions
2. `/user:backlog-list` — review what's accumulated in the global backlog
3. `/user:idea` — if starting a real project, this is the entry point
