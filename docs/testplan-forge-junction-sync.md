...
| `/install-forge` scenario detection | Skill logic is markdown — no executable under test |
...
| TC-003 | Migration preserves user data | Run migration via `/install-forge`; check `knowledge/`, `instincts/`, `tokens/`, `preferences.md` | All user dirs and files intact with original content | P1 |
| TC-004 | Migration flow end-to-end | From legacy copy-based install, run `/install-forge`; confirm it detects legacy, presents migration plan, creates junctions on MIGRATE | `~/.claude/skills/` becomes junction; user data untouched | P1 |
| TC-005 | `/update-forge` uses git pull | Run `/update-forge`; observe output | `git pull` executed; `update.sh` not mentioned or invoked | P1 |
| TC-006 | iOS guidance branch | Run `/install-forge` in environment without `ln`/`mklink` | iOS guidance displayed; no junction creation attempted | P2 |
...
| TC-008 | Wrong remote guard | With `~/forge` pointing to a different remote, run `/install-forge` | Error naming wrong remote; no filesystem changes | P2 |
...
| TC-010 | `update.sh` deprecation notice | Open `update.sh` | First comment block states file is deprecated; directs to `git pull` / `/update-forge` | P2 |
