...
3. **New skills** — `/onboard-knowledge` (guided end-to-end company setup) and `/check-style` (brand compliance review)

When complete, a developer onboarding to a new employer runs `/onboard-knowledge` once and is walked through the full setup in order, feeding source material in any format — Confluence URLs, SharePoint pastes, PDFs, Word docs, or verbal description.
...
4. As a developer, I want a `/check-style` skill that reviews any deliverable against the style guide, so that off-brand content is caught before sharing.
...
- **Sequence in `/onboard-knowledge`:** style-guide.md → acronyms → company domain terms → core systems (overview → schema → api → known-issues per system). Each stage is HITL-gated before the next begins.
- **Multi-source routing:** `/onboard-knowledge` and the extended `/summarise-system` detect source type at each step — URL (Confluence), file path (PDF/Word), or paste (SharePoint, other). The AI cannot access auth-gated URLs; paste is the fallback for SharePoint.
...
- **`/check-style` severity model:** mirrors `/check-pii` — CRITICAL (violates mandatory brand standard), HIGH (strong brand preference), LOW (suggestion).
...
- [ ] [HITL] #3 Review and confirm `/onboard-knowledge` SKILL.md draft before finalising
- [ ] [HITL] #4 Review and confirm `/check-style` SKILL.md draft before finalising
...
- [ ] [AFK] #8 Write `/onboard-knowledge` SKILL.md and command file `blocked-by: #3`
- [ ] [AFK] #9 Write `/check-style` SKILL.md and command file `blocked-by: #4`
...
- Test `/onboard-knowledge` with each source type in isolation: URL, file path, paste, verbal
- Test `/check-style` with a deliberately off-brand document — verify CRITICAL and HIGH findings are surfaced
...
- [ ] `/onboard-knowledge` walks through full sequence without errors on a clean `~/.claude/knowledge/`
- [ ] `/check-style` surfaces at least one finding on a test document
...
- `/onboard-knowledge` is the capstone — build it last so all the files it orchestrates already exist
