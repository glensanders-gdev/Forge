...
On multiple devices (Windows PC, Mac, iOS), each `~/.claude/` drifts independently. `/update-forge` closes the gap but only when run — and on Windows, the repo living under OneDrive causes junction/sync conflicts that make the junction-based fix impractical without moving the repo first.
...
5. As any Forge user, I want `/update-forge` to simply run `git pull`, so that staying up to date is a single command with no copy step.
...
- **`update.sh` deprecated** — retained in repo with a deprecation notice; no longer called by `/update-forge` or `install.sh`.
- **`/install-forge` auto-detects scenario** — checks whether `~/forge/.git` exists and whether `~/.claude/skills/` is already a junction/symlink. Three branches: fresh install, migration from existing copy-based install, already-linked (no-op).
...
- **iOS branch flow** — `/install-forge` detects iOS (no `mklink` or `ln` available), provides guidance: clone repo to working dir, work normally, push changes to `claude/ios-[slug]` branch, merge on Mac/Windows.
- **Module estimates:** `install.sh` rewrite S/3pts, `update.sh` deprecation S/1pt, `/install-forge` rewrite M/5pts, `/update-forge` simplification S/2pts.
...
- [ ] [AFK] #4 Rewrite `/install-forge` skill — auto-detect (fresh/migrate/already-linked), migration flow, iOS guidance branch `blocked-by: none`
- [ ] [AFK] #5 Simplify `/update-forge` skill — replace `update.sh` step with `git pull`, retain version check and CHANGELOG display `blocked-by: none`
- [ ] [HITL] #6 Run migration on Windows machine — execute updated `/install-forge`, confirm junctions resolve, verify skill edits appear in repo `blocked-by: #2, #4`
...
- Verify `/update-forge` runs `git pull` and reports version correctly without invoking `update.sh`
...
- [ ] `/update-forge` no longer references `update.sh`
