...
- **Token Recording sections removed** from 11 pipeline skills (build, deploy, estimate, grill-with-docs, idea, prototype, qa-plan, qa-report, research, testplan, write-prd) — recording moved to `/debrief` and `/end-sprint` via ccusage actuals. ~16 lines saved per skill, and removes a per-phase write ritual that cost far more than the lines themselves.
...
|-------|---------|-------|---------------------|------------|
| add-company | 8.8k | 873 | Once per employer | Oversized but rare — low priority |
...
| review-fy | 5.1k | 618 | Twice a year | Report templates could move to `references/` — low priority |
...
3. **add-company and review-fy — defer.** Both are oversized but run rarely; extract templates to `references/` only when next touched.
